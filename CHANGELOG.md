...
### Added
- **Fase 6 — Staging integral y preparación de producción:** entorno staging aislado en Vercel Preview con rama `staging/fase6-preproduction`. Protección mediante `NEXT_PUBLIC_NOINDEX=true`, `APP_ENV=staging`, Vercel SSO. Guardias reutilizables `lib/staging-guard.ts` (assertNotProduction, assertTestDatabase) y `lib/email-allowlist.ts` (redirección + marcado STAGING visual). Endpoints `/api/health` y `/api/health/readiness` con checks de DB, migraciones, Blob, cron, email e IA. Feature flags con deny-by-default, kill switches por scope y activación progresiva (olas 1-4). Email wrapper `lib/email-staging-wrapper.ts` con correlation ID y staging tag. Documentación operativa completa (8 archivos): topología, entorno, despliegue, observabilidad, backup/restore, rollback, seguridad, production readiness checklist y go/no-go. Lint reducido a 55 warnings (baseline 57). Certificación local completa: Vitest 3×1274 PASS, TypeScript 0 errors, Build PASS, Drizzle check OK.
...
### Fixed
- **Test inestable Fase 3 bajo paralelización:** `tests/fase3-experiencia-operativa.test.ts > WorkQueueService > returns array sorted by priority` fallaba por timeout (5000ms) cuando la suite completa corría en paralelo. Causa raíz: `await import()` de `lib/schema.ts`+`drizzle-orm` dentro del timer del primer `it` pagaba el coste de carga (~3.5s) bajo carga de CPU. Fix: `beforeAll` precarga los 9 módulos del test fuera del timer de cada `it`. Suite paralela ahora estable 3× (1113/1113). Sin timeouts ni reintentos.
- **Fase 4A — Certificación E2E real:** revalidación sobre HEAD `39f86b7` contra rama Neon aislada efímera `fase4a-cert-validation-20260720` (creada vía API Neon, eliminada al finalizar). E2E Fase 4A con DeepSeek real `deepseek-v4-flash` (latencia 1219ms, tipo identidad confianza 95): 19/19 assertions. Regresión E2E Fase 2: 9/9. Regresión E2E Fase 3: 70/70 (DeepSeek 527ms + Resend real con message ID persistido). Cero residuos de fixtures (prefijos `f2e2e`/`f3e2e`/`f4a`). Tests ampliados: `fase4a-feature-flags.test.ts` 16→24 (kill switch explícito auditado, deny-by-default, optimización `fetchApplicable`, precedencia), `fase4a-p2-05-p2-06-extended.test.ts` 32→41 (no-inventa-datos, aislamiento org, alerta SLA, plazo vencido, comunicación pendiente, sustitución por sourceHash). Suite serial 1065/1065. `.gitignore` +`*.tmp`. Fase 4A: **CERTIFICADA al 100%**.
- **Fase 4A — Cierre de bugs 5-8 (P2-06 y FeatureFlagService):** Bug 5 añadida fuente `det.firma_pendiente` en `next-action.ts` (detecta `eventos_agenda` tipo `firma` pendientes no cubiertos por la ventana de plazos, sin duplicar acción cuando la cubre `det.plazo_3dias`/`det.plazo_7dias`). Bug 6 `setFlag` usa transacción con `SELECT ... FOR UPDATE` + `INSERT ... ON CONFLICT DO NOTHING` + `UPDATE` fallback + historial en la misma transacción (resuelve race TOCTOU). Bug 7 `activateKillSwitch`/`deactivateKillSwitch` validan `settings.manage` vía `assertCapability` (deny-by-default, scope org, sin rol de cliente). Bug 8 `fetchApplicable` optimizado con cláusula `WHERE OR` por scope presente en el contexto (sin cambiar precedencia, kill switch ni deny-by-default). Tests ampliados: `tests/fase4a-p2-05-p2-06-extended.test.ts` de 16 a 32 tests (único checkpoint vigente, histórico, watermark, concurrencia/idempotencia, aislamiento, flag off, kill switch, firma, conflictos/prioridades, evidencias, DLQ aislado, autorización). Suite completa: 1048/1048 ✓. Sin cambios de schema (no migración nueva). ADR-010/011/012 añadidos.

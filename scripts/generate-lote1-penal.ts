...
const penalDocs = records.filter((r: any) => r.category === 'derecho-penal' && parseInt(r.impressions) > 0);
penalDocs.sort((a: any, b: any) => parseInt(b.impressions) - parseInt(a.impressions));

const lote1 = penalDocs.slice(0, 10);

const outReview = path.join(process.cwd(), 'docs', 'seo', 'review-packets', 'derecho-penal');
const outPatch = path.join(process.cwd(), 'data', 'seo', 'content-patches', 'derecho-penal');

fs.mkdirSync(outReview, { recursive: true });
fs.mkdirSync(outPatch, { recursive: true });

const mdLines = ['# Lote 1: Derecho Penal (Alta Impresión)\n'];
const patches: any[] = [];

for (const doc of lote1) {
  mdLines.push(`## ${doc.title_db}`);
  mdLines.push(`- URL: https://www.pinedayasociadoshn.com/blog/derecho-penal/${doc.slug}`);
...
fs.writeFileSync(path.join(outReview, 'lote1.md'), mdLines.join('\n'));
fs.writeFileSync(path.join(outPatch, 'lote1.json'), JSON.stringify(patches, null, 2));

console.log('Generated lote1 penal');

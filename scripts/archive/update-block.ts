...
async function main() {
  ...
  let pendingMatches = matches.filter(m => !m.isCompleted);
  
  // Limitar a un bloque de 5 artículos
  pendingMatches = pendingMatches.slice(0, 5);
  console.log(`Pendientes por procesar en este bloque: ${pendingMatches.length}`);

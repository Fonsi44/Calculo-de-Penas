...
function main() {
  ...
  (canonicalPaths.static_routes || canonicalPaths).forEach(p => {
  ...
  // 4. Bing
  bingData.priorityUrls.forEach(p => {
    const obj = getUrlObj(p.url);
    if (p.crawled && !p.crawled.startsWith('/Date(01/01/0001')) {
  ...
  for (const [pathUrl, data] of urls.entries()) {
    ...
    if (!pathUrl) {
      ...
      diagnostic = 'Indexable Correcta (Con tráfico)';
    } else if (data.isCanonical && data.gscImpressions === 0 && data.ga4Views === 0) {
    ...
    if (diagnostic.includes('Huérfano') && pathUrl) {
       // Check if it's trailing slash issue or weird params
  ...
  console.log('Indexability audit created.');
}

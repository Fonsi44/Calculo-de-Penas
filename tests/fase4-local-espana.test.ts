...
describe('FASE 4 §12 — FAQ visible coincide con schema', () => {
  it('el schema FAQPage de LandingLocalView usa las mismas faqs del dato', () => {
    const src = readRoot('components/marketing/landing-local.tsx');
    // El JSON-LD se construye a partir de landing.faqs (una sola fuente).
    expect(src).toMatch(/landing\.faqs\.map/);
  });
});

...
import { Plus, Trash2, Edit3, ChevronDown, ChevronRight, Save, X, ArrowUp, ArrowDown, ExternalLink } from 'lucide-react';
...
export default function AdminFaqPage() {
  ...
  const displayCategories = useMemo(() => {
    const fromDb = faqs.reduce<string[]>((acc, f) => { if (!acc.includes(f.category)) acc.push(f.category); return acc; }, []).sort();
    return fromDb.length > 0 ? fromDb : dbCategories.map(c => c.slug);
  }, [faqs, dbCategories]);
  ...
  const fetchFaqs = useCallback(() => {
    ...
    fetch(`/api/admin/faq?${params}`)
      .then(r => r.json())
      .then(data => { setFaqs(data.faqs ?? []); setGrouped(data.grouped ?? {}); })
  ...
  const filteredGrouped = useMemo(() => {
    ...
    return entries
      .map(([cat, entries]) => {
        ...
        if (q) { const lower = q.toLowerCase(); filtered = filtered.filter(f => f.question.toLowerCase().includes(lower) || (f.answer || '').toLowerCase().includes(lower)); }
  ...
  const allExpanded = () => setExpanded(new Set(filteredGrouped.map(([cat]) => cat)));
  ...
  const startEdit = (f: FaqEntry) => { setEditing(f.id); setEditForm({ category: f.category, question: f.question, answer: f.answer, sortOrder: f.sortOrder, published: f.published }); setShowNew(false); };
  const cancelEdit = () => { setEditing(null); setShowNew(false); };

  const saveEdit = async () => {
    if (!editing) return;
    if (!editForm.category || !editForm.question || !editForm.answer) { toast.danger('Todos los campos son obligatorios'); return; }
    setSaving(true);
    try {
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await fetch(`/api/admin/faq/${id}`, { method: 'DELETE' }); toast.success('FAQ eliminada'); fetchFaqs(); }
    catch { toast.danger('Error al eliminar'); }
  };

  const createFaq = async () => {
    if (!newForm.category || !newForm.question || !newForm.answer) { toast.danger('Todos los campos son obligatorios'); return; }
    if (!isCategoryValid(newForm.category)) { toast.danger('Categoría no válida.'); return; }
    setSaving(true);
    try {
      ...
      setShowNew(false); setNewForm({ category: '', question: '', answer: '' }); fetchFaqs();
    } catch (e) { toast.danger(e instanceof Error ? e.message : 'Error'); }
    finally { setSaving(false); }
  };
  ...
  const reorder = async (f: FaqEntry, direction: -1 | 1) => {
    try {
      await fetch(`/api/admin/faq/${f.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sortOrder: f.sortOrder + direction }) });
      fetchFaqs();
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="FAQ"
        subtitle={`${faqs.length} preguntas (${publishedCount} publicadas, ${draftCount} borradores)`}
        actions={
          <div className="flex gap-2">
            <Link href="/preguntas-frecuentes" target="_blank" rel="noopener noreferrer">
              <Button variant="ghost" size="sm"><ExternalLink size={14} /> Ver web</Button>
            </Link>
            <Button variant="primary" size="sm" onClick={() => { setShowNew(!showNew); setEditing(null); }}><Plus size={14} /> Nueva FAQ</Button>
          </div>
        }
      />
      ...
      {invalidCategories.length > 0 && (
        <Card padding="md" className="border-warning/50 bg-warning-bg">
          <p className="text-xs text-warning font-semibold">
            {invalidCategories.length} FAQ{invalidCategories.length > 1 ? 's' : ''} con categoría no reconocida: {[...new Set(invalidCategories.map(f => f.category))].join(', ')}.
          </p>
        </Card>
      )}
      ...
      <StatCards
        columns={3}
        items={[
          { value: faqs.length, label: 'Total', tone: 'default' },
          { value: publishedCount, label: 'Publicadas', tone: 'success' },
          { value: draftCount, label: 'Borradores', tone: 'warning' },
        ]}
      />
      ...
      <FilterBar
        ...
        filters={[
          { value: filterCategory, onChange: setFilterCategory, options: [{ value: '', label: 'Todas las categorías' }, ...displayCategories.map(c => ({ value: c, label: categoryName(c) }))] },
          { value: filterStatus, onChange: setFilterStatus, options: [{ value: 'all', label: 'Todos los estados' }, { value: 'true', label: 'Publicados' }, { value: 'false', label: 'Borradores' }] },
        ]}
        extraActions={
          <>
        ...
        <Card padding="md">
          ...
          <div className="space-y-3">
            <div>
              ...
              {newForm.category && !isCategoryValid(newForm.category) && <p className="text-xxs text-danger mt-1">Categoría no reconocida</p>}
            </div>
        ...
        <Card padding="lg">
          <div className="text-center space-y-2">
            <p className="text-text-secondary text-sm">
              {q || filterCategory || filterStatus !== 'all' ? 'No se encontraron FAQs con esos filtros.' : 'No hay FAQs aún.'}
            </p>
            {!q && !filterCategory && filterStatus === 'all' && faqs.length === 0 && (
              <Button variant="primary" size="sm" onClick={() => setShowNew(true)}><Plus size={14} /> Crear primera FAQ</Button>
            )}
          </div>
        </Card>
      ) : (
            ...
            {expanded.has(cat) && (
              <div className="border-t border-border">
                {entries.map(f => editing === f.id ? (
                  <div key={f.id} className="p-3 border-b border-border space-y-2 bg-surface-alt">
                    <label className="text-xs font-semibold text-text-secondary">Categoría</label>
                    ...
                    <div className="flex gap-2">
                      <Button onClick={saveEdit} variant="primary" size="sm" loading={saving}><Save size={14} /> Guardar</Button>
                      <Button onClick={cancelEdit} variant="ghost" size="sm"><X size={14} /> Cancelar</Button>
                    </div>
                  </div>
                ) : (
                  <div key={f.id} className="p-3 border-b border-border hover:bg-surface-alt flex items-start gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-text">{f.question}</p>
                        {f.published ? <Badge tone="success" className="flex-shrink-0">Público</Badge> : <Badge tone="warning" className="flex-shrink-0">Borrador</Badge>}
                      </div>

...
export default function AdminBibliotecaCP() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/cp?${params.toString()}`)
      ...
      .then((data: ArticuloResponse | ArticuloCP[]) => {
        if (Array.isArray(data)) { setArticulos(data); setTotal(data.length); }
        else { setArticulos(data.data); setTotal(data.total); }
      })
      .catch(() => {})
  ...
  useEffect(() => {
    fetch('/api/cp?count=1').then(r => r.json()).then(d => { if (typeof d.total === 'number') setTotal(prev => prev || d.total); }).catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <EmptyState icon={<BookOpen size={48} />} title="Sin resultados" description="Modifica la búsqueda o selecciona otro tema." />
      ) : (
        <>
          <div className="grid md:grid-cols-2 gap-2">
            {articulos.map(a => (
              <Card key={a.id} padding="none" className="hover:shadow-md transition-shadow">
                <Link href={`/intranet/admin/cp/${a.id}`} className="block p-3 focus-visible:outline-none">
                  <div className="flex items-start gap-2 mb-1.5">
                    <div className="min-w-0 flex-1">
                      <h3 className="font-bold text-sm text-text mb-1">{a.articulo}</h3>
                      {a.epigrafe && <p className="font-semibold text-xs text-text-secondary leading-4">{a.epigrafe}</p>}
                    </div>
                    <ArrowRight size={16} className="text-text-muted flex-shrink-0 mt-1" />
                  </div>
                  <p className="text-xs text-text-secondary line-clamp-3 leading-5 mb-2 font-serif">{a.texto}</p>

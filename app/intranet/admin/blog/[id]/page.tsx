...
const FUNNY_COMMENTS = [
  'Consultando el Código Penal...', 'Revisando jurisprudencia...',
  'Redactando como un abogado senior...', 'Buscando sinónimos jurídicos...',
  'Añadiendo palabras que impresionan al juez...', 'Verificando que no haya contradicciones legales...',
  'Citando doctrina... aunque sin decir cuál...', 'Formateando para que parezca muy profesional...',
  'Consultando con el equipo... (yo mismo)...', 'Echando café al teclado para que escriba más rápido...',
  'Convenciendo a las palabras de que se ordenen solas...', 'Puliendo el SEO como un diamante en bruto...',
  'Insertando latinajos para darle seriedad...',
];

interface GeneratedPost {
  title: string; description: string; body: string; slug: string;
  readingTime: string; tags: string[]; category: string;
}
...
export default function AdminBlogEditorPage() {
  ...
  const handleBodyChange = useCallback((html: string) => {
    ...
    if (activeTab === 'visual') setCodeValue(html);
  }, [activeTab]);

  const handleCodeChange = useCallback((code: string) => {
    setCodeValue(code);
    setCodeError(null);
    try { const div = document.createElement('div'); div.innerHTML = code; setForm(f => ({ ...f, body: code })); }
    catch { setCodeError('El HTML contiene errores de sintaxis'); }
  }, []);

  const switchToVisual = useCallback(() => { setForm(f => ({ ...f, body: codeValue })); setActiveTab('visual'); }, [codeValue]);
  const switchToCode = useCallback(() => { setCodeValue(form.body); setCodeError(null); setActiveTab('code'); }, [form.body]);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) { toast.danger('Solo se permiten imágenes JPEG, PNG o WebP'); return; }
    if (file.size > 10 * 1024 * 1024) { toast.danger('La imagen no puede superar los 10 MB'); return; }
    setUploading(true);
    try {
    ...
    } catch (err) { toast.danger(err instanceof Error ? err.message : 'Error al subir imagen'); }
    finally { setUploading(false); }
  };

  const clearCoverImage = () => { update('coverImage', ''); setUploadPreview(null); };
  ...
  const handleGeneratePost = async () => {
    ...
    try {
      ...
      setForm({
        slug: post.slug, title: post.title, description: post.description, body: post.body,
        publishedAt: new Date().toISOString().slice(0, 16), category: post.category,
        tags: post.tags.join(', '), author: 'Pineda y Asociados', readingTime: post.readingTime,
      ...
      setGenTopic('');
    } catch (e) { toast.danger(e instanceof Error ? e.message : 'Error al generar'); setFunnyComment(''); }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {isDirty && <p className="text-xxs text-warning flex items-center gap-1"><AlertTriangle size={10} /> Cambios sin guardar</p>}
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <Button type="button" variant="primary" size="sm" onClick={(e) => handleSave(e as unknown as React.FormEvent)} loading={saving}>
            <Save size={14} /> {isNew ? 'Publicar' : 'Actualizar'}
          </Button>
        </div>
      </div>
      ...
      {isNew && (
        <Card padding="md">
          ...
          <p className="text-xs text-text-secondary mb-3">Escribe un tema y selecciona la categoría. El sistema generará un artículo completo con estructura legal.</p>
          <div className="flex flex-wrap gap-2 mb-3">
            <div className="flex-1 min-w-[200px]">
              <Input value={genTopic} onChange={e => setGenTopic(e.target.value)} placeholder="Tema del artículo, ej: Cómo reclamar una herencia en Honduras" />
            </div>
            ...
            <Button onClick={handleGeneratePost} variant="primary" size="sm" loading={generating} disabled={generating}>
              {generating ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />} Generar post
            </Button>
          </div>
          {generating && (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <div className="flex-1 h-2 bg-surface-alt rounded-full overflow-hidden">
                  <div className="h-full bg-accent rounded-full transition-all duration-300" style={{ width: `${genProgress}%` }} />
                </div>
                <span className="text-xs font-bold text-accent tabular-nums">{Math.round(genProgress)}%</span>
              </div>
              <p className="text-xs text-text-secondary italic flex items-center gap-1"><Sparkles size={12} className="text-accent" />{funnyComment}</p>
            </div>
          )}
          {!generating && form.body && (
            <div className="flex items-center gap-2 text-xs text-success"><CheckCircle2 size={14} />Artículo generado y cargado en el editor</div>
          )}
        </Card>
      )}
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex rounded-md border border-border overflow-hidden">
                  <button type="button" onClick={() => activeTab === 'code' ? switchToVisual() : undefined}
                    className={cn('flex items-center gap-1 px-3 py-1 text-xs font-medium transition-colors', activeTab === 'visual' ? 'bg-accent/15 text-primary' : 'text-text-secondary hover:bg-surface-alt')}>
                    <Eye size={12} /> Editor visual
                  </button>
                  <button type="button" onClick={() => activeTab === 'visual' ? switchToCode() : undefined}
                    className={cn('flex items-center gap-1 px-3 py-1 text-xs font-medium transition-colors', activeTab === 'code' ? 'bg-accent/15 text-primary' : 'text-text-secondary hover:bg-surface-alt')}>
              ...
              {activeTab === 'visual' ? (
                ...
                <div className="space-y-1">
                  <textarea value={codeValue} onChange={e => handleCodeChange(e.target.value)}
                    className="w-full min-h-[400px] p-3 rounded-md border border-border bg-surface font-mono text-xs text-text leading-relaxed outline-none focus:ring-2 focus:ring-accent/30 resize-y"
                    spellCheck={false} placeholder="<p>Escribe el contenido HTML aquí...</p>" />
                  {codeError && <p className="text-xs text-danger flex items-center gap-1"><AlertTriangle size={10} /> {codeError}</p>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <select value={form.category} onChange={e => update('category', e.target.value)}
                className="w-full h-9 rounded-md border border-border bg-surface px-2.5 text-sm text-text outline-none transition-all hover:border-border-strong focus:border-accent focus:shadow-[0_0_0_3px_rgba(212,175,55,0.18)]">
                {dbCategories.map(c => <option key={c.slug} value={c.slug}>{c.nombre}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button type="button" onClick={handleAutoTags} className="text-xxs text-accent hover:text-accent-dark flex items-center gap-1 transition-colors"><Tag size={11} /> Auto</button>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button type="button" onClick={handleAutoReadingTime} className="text-xxs text-accent hover:text-accent-dark flex items-center gap-1 transition-colors"><Clock size={11} /> Auto</button>
              </div>
            ...
            <div>
              ...
              <div className="mt-2">
                <label className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border bg-surface hover:bg-surface-alt text-xs text-text-secondary cursor-pointer transition-colors">
                ...
                {form.coverImage && (
                  <button type="button" onClick={clearCoverImage} className="ml-2 inline-flex items-center gap-1 text-xxs text-danger hover:text-danger-dark transition-colors"><X size={11} /> Quitar</button>
                )}
              </div>
              ...
              <p className="text-xxs text-text-muted mt-1">Formatos: JPEG, PNG, WebP. Máx. 10 MB.</p>
            </div>
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={form.featured} onChange={e => update('featured', e.target.checked)} className="w-4 h-4 rounded border-border text-accent focus:ring-accent/30" /> Destacado
            </label>
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={form.published} onChange={e => update('published', e.target.checked)} className="w-4 h-4 rounded border-border text-accent focus:ring-accent/30" /> Publicado
            </label>
            <div className="flex flex-col gap-2 pt-2 border-t border-border">
              ...
              <Button type="button" variant="primary" size="sm" onClick={(e) => handleSave(e as unknown as React.FormEvent)} loading={saving} fullWidth>
                <Save size={14} /> {isNew ? 'Publicar post' : 'Actualizar post'}
              </Button>
            </div>
          </div>
        </div>

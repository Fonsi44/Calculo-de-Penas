...
interface Usuario { id: string; email: string; nombre: string; rol: string; creadoEn: string; }
...
export default function AdminUsuariosPage() {
  ...
  useEffect(() => { fetchUsuarios(); }, [page]); // eslint-disable-line react-hooks/exhaustive-deps,react-hooks/set-state-in-effect

  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); setPage(1); fetchUsuarios(); };
  ...
  const handleDelete = async (id: string, nombre: string) => {
    if (!await confirm({ title: `¿Eliminar a ${nombre}?`, description: 'Esta acción no se puede deshacer.' })) return;
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <Card padding="md"><p className="text-center text-text-secondary text-sm">No se encontraron usuarios</p></Card>
      ) : (
        <Card padding="none">
                  ...
                  <tr key={u.id} className="border-b border-border hover:bg-surface-alt">
                    ...
                    <td className="p-3">
                      <Badge tone={u.rol === 'admin' ? 'warning' : 'neutral'}>
                        {u.rol === 'admin' ? <Shield size={10} className="mr-1" /> : <User size={10} className="mr-1" />}{u.rol}
                      </Badge>
                    </td>

...
export default function AdminEditarUsuarioPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/usuarios?limit=100`)
      .then(r => r.json())
      .then(data => {
        const u = (data.usuarios ?? []).find((x: { id: string }) => x.id === params.id);
        if (u) setForm({ nombre: u.nombre, email: u.email, rol: u.rol, password: '' });
      })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-extrabold text-primary">Editar Usuario</h1>
      </div>
      <Card padding="md">

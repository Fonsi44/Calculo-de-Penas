...
import { downloadArtifacts, signatureServiceErrorResponse } from '@/lib/sgie/signature-service';
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string; packageId: string; envelopeId: string }> }) {
  try {
    ...
    const result = await downloadArtifacts(envelopeId, { actorId: auth.userId });
